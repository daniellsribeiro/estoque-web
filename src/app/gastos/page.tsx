...
export default function GastosPage() {
  ...
  const normalizeErrorMessage = (err: unknown, fallback = "Erro ao salvar gasto"): string => {
    const fromResponse = (err as { response?: { message?: unknown } } | null | undefined)?.response?.message;
    const message = (err as { message?: unknown } | null | undefined)?.message ?? fromResponse;
  ...
  const buildGastosQuery = useCallback(
    ...
    [filtroDescricao, filtroDataFim, filtroDataInicio, filtroFornecedorId, filtroStatus, filtroTipoPagamentoId],
  );
  ...
  return (
    <div>
      <PageMeta title="Gastos" subtitle="Despesas operacionais e pagamentos em cartão/PIX." />
      {message && (
      ...
      {mostrarNovoGasto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 px-4">
          <div className="w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-xl bg-slate-900 p-6 ring-1 ring-slate-800">
